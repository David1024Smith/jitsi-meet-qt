...
bool MetricsCollector::registerMonitor(BaseMonitor* monitor)
{
    ...
    connect(monitor, &BaseMonitor::dataUpdated,
            this, [this, name](const ResourceUsage& usage) {
                QVariantMap data;
                data["timestamp"] = usage.timestamp;
                data["cpu_usage"] = usage.cpu.totalUsage;
                data["memory_usage"] = usage.memory.usagePercentage;
                data["disk_usage"] = usage.disk.usagePercentage;
                data["network_received"] = usage.network.bytesReceived;
                data["network_sent"] = usage.network.bytesSent;
                handleMonitorData(name, data);
            });

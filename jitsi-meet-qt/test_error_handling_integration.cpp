...
public:
    ErrorHandlingIntegrationTest(QObject* parent = nullptr) : QObject(parent) {}
...
private slots:
    ...
    void testXMPPErrorHandling();
    void testWebRTCErrorHandling();
    void testMediaDeviceErrorHandling();
...
void ErrorHandlingIntegrationTest::logTestResult(const QString& testName, bool passed, const QString& details)
{
...
void ErrorHandlingIntegrationTest::testJitsiErrorSystem()
{
...
void ErrorHandlingIntegrationTest::testErrorRecoveryManager()
{
...
void ErrorHandlingIntegrationTest::testErrorUtils()
{
    qDebug() << "\n=== Testing ErrorUtils ===";
...
void ErrorHandlingIntegrationTest::testErrorDialog()
{
    qDebug() << "\n=== Testing ErrorDialog ===";
    
    // Test 1: Dialog creation (without showing)
    try {
        JitsiError error = JitsiError::networkError("Test network error", "Connection timeout");
        ErrorDialog dialog(error);
        
        bool test1 = (dialog.result() == ErrorDialog::Cancel);
        
        logTestResult("Dialog creation", test1);
    } catch (...) {
        logTestResult("Dialog creation", false, "Exception thrown");
    }
    
    // Test 2: Dialog configuration
    try {
        JitsiError error = JitsiError::configurationError("Config error");
        ErrorDialog dialog(error);
        
        dialog.setAutoCloseTimeout(10);
        dialog.setRetryEnabled(true);
        dialog.setResetEnabled(true);
        dialog.setShowDetails(false);
        
        bool test2 = true; // If no exception, configuration worked
        
        logTestResult("Dialog configuration", test2);
    } catch (...) {
        logTestResult("Dialog configuration", false, "Exception thrown");
    }
}

void ErrorHandlingIntegrationTest::testNetworkErrorHandling()
{
...
void ErrorHandlingIntegrationTest::testXMPPErrorHandling()
{
    qDebug() << "\n=== Testing XMPP Error Handling ===";
    ...
    try {
        ...
        bool test1 = (xmppError.type() == ErrorType::XMPPConnectionError) &&
                     (result.strategy == ErrorRecoveryManager::RecoveryStrategy::Retry);
        
        logTestResult("XMPP error handling", test1);
    } catch (...) {
        logTestResult("XMPP error handling", false, "Exception thrown");
    }
}

void ErrorHandlingIntegrationTest::testWebRTCErrorHandling()
{
    qDebug() << "\n=== Testing WebRTC Error Handling ===";
    ...
    try {
        ...
        bool test1 = (webrtcError.type() == ErrorType::WebRTCError) &&
                     (result.strategy == ErrorRecoveryManager::RecoveryStrategy::Restart);
        
        logTestResult("WebRTC error handling", test1);
    } catch (...) {
        logTestResult("WebRTC error handling", false, "Exception thrown");
    }
}

void ErrorHandlingIntegrationTest::testMediaDeviceErrorHandling()
{
    qDebug() << "\n=== Testing Media Device Error Handling ===";
    ...
    try {
        ...
        bool test1 = (mediaError.type() == ErrorType::MediaDeviceError) &&
                     (result.strategy == ErrorRecoveryManager::RecoveryStrategy::Reset);
        
        logTestResult("Media device error handling", test1);
    } catch (...) {
        logTestResult("Media device error handling", false, "Exception thrown");
    }
}

void ErrorHandlingIntegrationTest::testAuthenticationErrorHandling()
{
    ...
    try {
        ...
        bool test1 = (authError.type() == ErrorType::AuthenticationError) &&
                     (result.strategy == ErrorRecoveryManager::RecoveryStrategy::UserIntervention);
        
        logTestResult("Authentication error handling", test1);
    } catch (...) {
...
int main(int argc, char *argv[])
{
    ...
    ErrorHandlingIntegrationTest test;
    
    // Run all tests
    test.testJitsiErrorSystem();
    test.testErrorRecoveryManager();
    test.testErrorUtils();
    test.testErrorDialog();
    test.testNetworkErrorHandling();
    test.testXMPPErrorHandling();
    test.testWebRTCErrorHandling();
    test.testMediaDeviceErrorHandling();
    test.testAuthenticationErrorHandling();
